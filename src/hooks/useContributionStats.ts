...
import type { Contribution as DirectusContribution, ContributionsStatus } from "../../models/types";
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const items = await directus.request<(
        Pick<DirectusContribution, "date_created"> & { status: Pick<ContributionsStatus, "label"> }
      )[]>(
      ...
      items.forEach((item) => {
        ...
        if (!weeklyByStatus[week]) weeklyByStatus[week] = {};
      ...
      return { donutData, lineData, barData } as ContributionStats;
    },
  });
}
