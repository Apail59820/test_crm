...
export default function TiptapMenuBar({ editor }: { editor: Editor | null }) {
  ...
  const colorMenu: MenuProps = {
    items: colors.map((c) => ({
      ...
      onClick: () =>
        (editor?.chain().focus() as { setColor: any }).setColor(c).run(),
    })),
  };
  ...
  return (
    <Space wrap size="small" style={{ marginBottom: 8 }}>
      ...
      <Tooltip title="Souligné">
        <Button
          icon={(<UnderlineOutlined />) as any}
          type={editor.isActive("underline") ? "primary" : "default"}
          onClick={() =>
            (editor?.chain().focus() as { toggleUnderline: any })
              .toggleUnderline()
              .run()
          }
        />
      ...
      <Tooltip title="Barré">
        <Button
          icon={(<StrikethroughOutlined />) as any}
      ...
      <Tooltip title="Liste à puces">
        <Button
          icon={(<UnorderedListOutlined />) as any}
      ...
      <Dropdown menu={colorMenu} trigger={["click"]}>
        <Button icon={(<FontColorsOutlined />) as any} />
      </Dropdown>

      <Tooltip title="Surlignage">
        <Button
          icon={(<HighlightOutlined />) as any}
          type={editor.isActive("highlight") ? "primary" : "default"}
          onClick={() =>
            (editor?.chain().focus() as { toggleHighlight: any })
              .toggleHighlight()
              .run()
          }
        />
      ...
      <Tooltip title="Lien">
        <Button
          icon={(<LinkOutlined />) as any}
          type={editor.isActive("link") ? "primary" : "default"}
          onClick={() => {
            const url = prompt("URL ?");
            if (url) {
              (
                editor?.chain().focus().extendMarkRange("link") as {
                  setLink: any;
                }
              )
                .setLink({ href: url })
